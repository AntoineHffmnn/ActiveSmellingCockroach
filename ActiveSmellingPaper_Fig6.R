...
Peaks.list<-lapply(fulllist,
                        FUN=function(df){ 
                          ...
                          # #max contrast
                          # peak.contrasts<-peak.contrasts[peak.contrasts>=0] #positive contrasts only
                          # maxcontrast<-max(peak.contrasts,na.rm=T)
                          # if(is.infinite(maxcontrast)==T){ maxcontrast<-0 } #no peaks, no contrast
                          ...
                          # # Derivative
                          # d<-diff(df$pidvoltage)
                          # # peaks
                          # dpeaks<-peaks(time=(1:length(d)),var=d,halfwin=3,thresh=0.03)
                          # # reformat peaks
                          # dpeaks[dpeaks==-1]<-1 #all peaks
                          # # dpeaks[dpeaks!=1]<-0 #only keep positive peaks
                          # # rate
                          # peakrate<-rate.calc(peakvect=dpeaks,method="instantaneous")
                          # meanpeakrate<-mean(peakrate,na.rm=T) #mean in the window
                          
                          
                          #avg angular speed in that window
                          # avgspeed<-unique(na.omit(df$WindowMeanSpeed))
                          # avgspeed<-mean(df$ResultingAngularSpeed,na.rm=T)
                          ...
                          metrics<-data.frame("window"=paste(unique(na.omit(df$Test)),min(df$Count),sep="_"),
                                              # "meanderivpeakrate"=meanpeakrate,
